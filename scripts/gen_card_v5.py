...
用法:
    python3 scripts/gen_card_v5.py                # 从 BUDDY_KEY 查询并生成
    python3 scripts/gen_card_v5.py --data <json>  # 直接传入积分数据 JSON
    python3 scripts/gen_card_v5.py --rss <xml>    # 从 rss.xml 解析数据（离线预览调试）
环境变量:
...
def draw_card(data: dict, now: datetime.datetime, out_path: str = "card_v5.png"):
    ...
    # 字体: [标题(15), 左栏大数字(44), 左栏标签(10), 左栏格子数值(22), 右栏标签(10), 右栏数值(16)]
    ...
    print(f"✅ 卡片 v5 已生成: {out_path} ({W}x{H})")
...
def main():
    ...
    parser.add_argument("--out", default="card_v5.png", help="输出路径")
