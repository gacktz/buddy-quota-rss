...
用法:
    python3 scripts/gen_card_v6.py                # 从 BUDDY_KEY 查询并生成
    python3 scripts/gen_card_v6.py --data <json>  # 直接传入积分数据 JSON
    python3 scripts/gen_card_v6.py --rss <xml>    # 从 rss.xml 解析数据（离线预览调试）
环境变量:
...
def draw_card(data: dict, now: datetime.datetime, out_path: str = "card_v6.png"):
    ...
    for i, (label, value) in enumerate(left_cells):
        ...
        # 数值（垂直居中于下半部）
        f_val = fit_font(d, f_left_val, value, cell_w - 6, min_size=12)
    ...
    for i, (label, value) in enumerate(cells):
        ...
        f_val = fit_font(d, f_rc_val, value, col_w - 8, min_size=11)
        d.text((x0 + 4, y1 - 17), value, font=f_val, fill=BLACK)

    img.save(out_path, "PNG")
    print(f"✅ 卡片 v6 已生成: {out_path} ({W}x{H})")
...
def main():
    ...
    parser.add_argument("--out", default="card_v6.png", help="输出路径")
