...
# -*- coding: utf-8 -*-
"""
Buddy 积分 → 墨水屏卡片生成脚本 v7（小象专属版 · 布局修正版）
- 296×152 纯黑白 PNG（Quote/0 图像 API 全屏规格）
- 布局：标题行 + 左右分栏
  左栏（余额区）：剩余积分超大数字 + 已用进度条 + 已用百分比
                  下方两个指标格：今日消耗 / 今日请求
  右栏（详情区）：2×2 指标网格（余额包/累计已用/累计充值/更新时间）
- v5 改进：粗体字体优先（墨水屏黑白渲染更清晰）+ 字号全面加大
- v7 改进：数值水平居中 + 垂直居中于格子内，修正长数字贴边/越界问题；
           左栏百分比与下方格子留白，避免文字压线

用法:
    python3 scripts/gen_card_v7.py                # 从 BUDDY_KEY 查询并生成
    python3 scripts/gen_card_v7.py --data <json>  # 直接传入积分数据 JSON
    python3 scripts/gen_card_v7.py --rss <xml>    # 从 rss.xml 解析数据（离线预览调试）
环境变量:
...
def draw_card(data: dict, now: datetime.datetime, out_path: str = "card_v7.png"):
    ...
    # 字体: [标题(15), 左栏大数字(44), 左栏标签(10), 左栏格子数值(22), 右栏标签(10), 右栏数值(16), 右上角标识(13)]
    f_title, f_bal, f_lbl, f_left_val, f_rc_lbl, f_rc_val, f_badge = find_font(
        [15, 44, 10, 22, 10, 16, 13])

    # ---- 标题行 ----
    d.text((MARGIN, 4), "BUDDY 积分", font=f_title, fill=BLACK)
    d.text((W - MARGIN, 5), version, font=f_badge, fill=BLACK, anchor="ra")
    ...
    # 今日指标两个格子（左栏下方）
    cell_y0, cell_y1 = 104, 146   # v7: 下移留白，避免与上方百分比文字压线
    gap = 5
    ...
    for i, (label, value) in enumerate(left_cells):
        ...
        # 数值：水平居中 + 垂直居中于标签下方到格子底部
        bbox_l = d.textbbox((0, 0), label, font=f_lbl)
        lbl_bottom = cell_y0 + 2 + (bbox_l[3] - bbox_l[1])
        ...
        y_mid = (lbl_bottom + cell_y1) / 2
        d.text((x0 + (cell_w - vw) / 2, y_mid - vh / 2), value,
    ...
    for i, (label, value) in enumerate(cells):
        ...
        # 数值：水平居中 + 垂直居中于标签下方到格子底部（v7 修复贴边/越界）
        bbox_l = d.textbbox((0, 0), label, font=f_rc_lbl)
        lbl_bottom = y0 + 3 + (bbox_l[3] - bbox_l[1])
        f_val = fit_font(d, f_rc_val, value, col_w - 4, min_size=8)
        bbox_v = d.textbbox((0, 0), value, font=f_val)
        vw, vh = bbox_v[2] - bbox_v[0], bbox_v[3] - bbox_v[1]
        y_mid = (lbl_bottom + y1) / 2
        d.text((x0 + (col_w - vw) / 2, y_mid - vh / 2), value,
               font=f_val, fill=BLACK)

    img.save(out_path, "PNG")
    print(f"✅ 卡片 v7 已生成: {out_path} ({W}x{H})")
...
def main():
    ...
    parser.add_argument("--out", default="card_v7.png", help="输出路径")
